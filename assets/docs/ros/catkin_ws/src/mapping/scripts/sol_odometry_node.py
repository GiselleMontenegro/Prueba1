...
def process_data():
    ...
    # curren_time stores the time of the current iteration
    current_time = rospy.Time.now()
    # _last_time stores the time of the iteration before the current iteration
    ...
    dt = (current_time - _last_time).to_sec()
    delta_x = (_cmd_vel.linear.x * cos(_odometry.theta)) * dt
    delta_y = (_cmd_vel.linear.x * sin(_odometry.theta)) * dt
    delta_th = _cmd_vel.angular.z * dt
    ...
    _odometry.theta = _odometry.theta + delta_th
    if _odometry.theta > pi:
        _odometry.theta = -pi + (_odometry.theta - pi)
    elif _odometry.theta < -pi:
        _odometry.theta = pi - (_odometry.theta + pi)
    # Saving current_time in _last_time for next iteration
    _last_time = current_time
