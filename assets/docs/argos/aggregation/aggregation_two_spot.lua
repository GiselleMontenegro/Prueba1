...
BASE_LEAVE_PROB = 0.5 -- the basic probability to stay. It should be small enough to avoid splitting the group, but big enough to achieve stability
STOP_PROB = 0.2
LEAVE_STEPS = 10 -- when leaving we go straight for this number of steps
...
function ProcessProx()
   ...
   if sort_prox[1].value > 0.02 and math.abs(sort_prox[1].angle) < math.pi/2
